...
interface CreateCollectionModalProps {
  onCreateCollection: (collection: CreateCollectionType) => void;
...
const CreateCollectionDialogTrigger = ({
  ...
  const [randomDestinations, setRandomDestinations] = useState<DestinationType[]>([]);
  ...
  const onSubmit = async (values: CreateCollectionType) => {
    // You can add loading handling here if needed
  ...
  useEffect(() => {
    const getRandomDestinations = async () => {
      try {
        const res = await axios.get<DestinationType[]>("/api/destinations/random");
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Organize your favorite places into a themed collection to share with others or keep for yourself.
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-7">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g., Beach Paradise, Mountain Adventures" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground text-sm">
                Choose some places to get started. You can add more or edit later.
              </p>
              </div>
              ...
              <div className="rounded-xl border py-1">
                <div className="grid max-h-52 gap-3 overflow-y-auto rounded-xl px-2 py-1 sm:grid-cols-2">
                  {randomDestinations.map((destination) => {
                    const isSelected = form.watch("destinations").includes(destination._id);
                    return (
                      <Card
                        key={destination._id}
